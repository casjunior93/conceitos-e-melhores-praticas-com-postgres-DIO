---Common Table Expression - resultados temporarios para consultas coplexas

SELECT numero, nome FROM banco;
SELECT banco_numero, numero, nome FROM agencia;

WITH tabela_temp_banco AS (
	SELECT numero, nome
	FROM banco
)
SELECT numero, nome
FROM tabela_temp_banco;

---

WITH params AS (
	SELECT 213 AS banco_numero
), tabela_temp_banco AS (
	SELECT numero, nome
	FROM banco
	JOIN params ON params.banco_numero = banco.numero
)
SELECT numero, nome FROM tabela_temp_banco;
